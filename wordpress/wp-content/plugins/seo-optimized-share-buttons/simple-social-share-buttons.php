...
/*
  Plugin Name: Social Share Buttons - SEO Optimized
  Plugin URI: 
  Description: Follow the new guidelines from Google with our social share buttons so you don't get punished and drop in rank! We have done all the work for you.
  Version: 1.0
  Author: spyrosli
  Author URI: 
 ...
 Thanks to: AccessPress Social Share version 4.2.2 by Access Keys: https://wordpress.org/plugins/accesspress-social-share/ and FontAwesome http://fontawesome.io/
 Read more about credits, copyright and GPLv2 license in readme.txt 
...
if ( !class_exists( 'APSS_Class' ) ) {
	...
	class APSS_Class {
		...
		function apss_the_content_filter( $content ) {
			...
			if ( empty( $options['share_options'] ) ) {
				...
				if ( $options['share_positions'] == 'below_content' ) {
					return $post_content . "<div class='apss-social-share apss-theme-4 clearfix' >" . $html_content . "</div>";
				}

				if ( $options['share_positions'] == 'above_content' ) {
					return "<div class='apss-social-share apss-theme-4 clearfix'>$html_content</div>" . $post_content;
				}

				if ( $options['share_positions'] == 'on_both' ) {
					return "<div class='apss-social-share apss-theme-4 clearfix'>$html_content</div>" . $post_content . "<div class='apss-social-share apss-theme-$icon_set_value clearfix'>$html_content</div>";
				}
			} else {
				return $post_content;
			}
		...
		function add_apss_menu() {
			add_menu_page( 'Share Buttons', 'Share Buttons', 'manage_options', 'accesspress-social-share', array( $this, 'main_page' ), APSS_IMAGE_DIR . '/apss-icon.png' );
		...
		function social_meta_box() {
			add_meta_box( 'ap-share-box', '', array( $this, 'metabox_callback' ), '', 'side', 'core' );
		}
		function metabox_callback( $post ) {
			wp_nonce_field( 'save_meta_values', 'ap_share_meta_nonce' );
			$content_flag = get_post_meta( $post->ID, 'apss_content_flag', false );
			?>
			<?php
		}
